...
- **`to-review` 스킬** — 내가 리뷰어로 등록된 열린 PR을 한눈에 보여줍니다. PR마다 본문에서 뽑은 한줄요약, 리뷰가 요청된 시점부터의 대기 시간, 변경 파일 규모(파일 수 + 증감 라인)를 카드형 리스트로 정리하며, 대기가 오래된 순으로 보여줍니다. 조회·요약 전용이라 GitHub에 코멘트/approve를 남기지 않습니다. (GitHub CLI(`gh`) 로그인 필요)
...
설치가 끝나면 `surface`·`to-review` 스킬과 `communicator` 에이전트가 활성화됩니다.
...
- **`/to-review`** — "내가 리뷰할 PR 목록 보여줘", "리뷰 대기 얼마나 됐지" 같은 상황에서 실행합니다. 리뷰 요청받은 PR을 요약·대기시간·변경규모와 함께 대기 오래된 순으로 정리해 줍니다.
